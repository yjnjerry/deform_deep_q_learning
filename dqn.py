...
import gym
import sys
sys.path.append('/home/jinbeibei/code/python/dqn-master/deform_conv')
import random
import numpy as np
...
class Agent():
    ...
    def build_network(self):
        model = Sequential()
        print(STATE_LENGTH,FRAME_HEIGHT,FRAME_WIDTH)
        #model.add(Convolution2D(32, 8, 8, subsample=(4, 4),border_mode='same', activation='relu', input_shape=(STATE_LENGTH, FRAME_WIDTH, FRAME_HEIGHT)))
       ...
       # model.add(Dense(self.num_actions))
        ...
        #inputs = l = Input(( FRAME_HEIGHT, FRAME_WIDTH,STATE_LENGTH), name='input')
        model.add(Conv2D(32, (8, 8), subsample=(4, 4), border_mode='same', name='conv11',input_shape=(STATE_LENGTH,FRAME_HEIGHT, FRAME_WIDTH)))
        model.add(Activation('relu', name='conv11_relu'))
        #model.add(BatchNormalization(name='conv11_bn'))

        model.add(ConvOffset2D(32, name='conv12_offset'))
        model.add(Conv2D(64, (4, 4), subsample=(2, 2), border_mode='same', name='conv12' ))
        model.add(Activation('relu', name='conv12_relu'))
        #model.add(BatchNormalization(name='conv12_bn'))

        model.add(ConvOffset2D(64, name='conv21_offset'))
        model.add(Conv2D(64, (3, 3), subsample=(1, 1), border_mode='same', name='conv21' ))
        model.add(Activation('relu', name='conv21_relu'))
        #model.add( BatchNormalization(name='conv21_bn'))
        model.add(Flatten())
        model.add(Dense(512, activation='relu'))
        model.add(Dense(self.num_actions))
